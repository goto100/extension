...
function require2(request) {
	...
	roots.forEach(function(root) {
		var filename;
		root = fs.realpathSync(root); // 避免由于符号链接路径不一致，导致忽略外部路径引用时判断错误。
	...
	if (notFound) {
		...
		targetModule.extended = [];
		if (targetModule.extensions) {
			targetModule.extensions.forEach(function(ext, i) {
				cache[ext].extending = targetModule;
				require(ext);
				targetModule.extended[i] = require.cache[ext];
			});
		}
