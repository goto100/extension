 ...
 * 包装一个module的require
 */
function wrap(hostModule, extensions) {
	...
	return function(request) {
		...
		if (isNew) {
			mods = extRequire(request, hostModule, exts);
			// 多个扩展都返回了这个模块，冲突报错。
			if (mods.length > 1) {
		...
		// 加载扩展中的同名模块
			...
			// require所有扩展中的同名模块
			extRequire(request, hostModule, exts);
		}
		return expts;
	};
}
 ...
 */
function extRequire(request, hostModule, exts) {
	var mods = [];
	// 只hook相对路径
...
function patchRequire() {
	...
	require.extensions['.js'] = function(module, filename) {
		// 所有模块都可以直接require到根目录，主要为扩展提供支持
